...
const staged_files_path = path.join(__dirname, ".staged_files");
const exclusions_path = path.join(__dirname, ".converter_exclusions");
const converted_source_image_files_path = path.join(__dirname, ".converted_source_image_files");
const converted_target_image_files_path = path.join(__dirname, ".converted_target_image_files");
const error_path = path.join(__dirname, ".converter_errors");
...
try {
    ...
    if ((await fsp.stat(converted_source_image_files_path).catch(() => false)) !==
        false) {
        await fsp.unlink(converted_source_image_files_path);
    }
    if ((await fsp.stat(converted_target_image_files_path).catch(() => false)) !==
        false) {
...
class ImageConverter {
    ...
    getOptions(argv) {
        ...
        program
            ...
            .option("--override-excluded-files", "don't exclude files from exclusions list", false)
            .option("--create-fallback-image", "create a PNG/GIF file if a JPEG, PNG, GIF, TIFF, or BMP file doesn't exist", false)
            .parse(argv);
        return program;
    }
    async runProgram() {
    ...
    async processFiles(files) {
        try {
            ...
            if (!this.options.overrideExcludedFiles) {
                exclusions = new RegExp((await fsp.readFile(exclusions_path, "utf-8"))
                    .trim()
                    .replace(/[\r\n]/g, "|"));
            }
            ...
            if (!this.options.overrideStagedFiles &&
                ...
                if (exclusionsString !== "/(?:)/") {
                    filesFiltered = files.filter((filepath) => stagedFilesList?.includes(filepath) === true &&
                        exclusions?.test(filepath) === false);
                }
                else {
                    filesFiltered = files.filter((filepath) => stagedFilesList?.includes(filepath));
                }
            }
                ...
                if (exclusionsString !== "/(?:)/") {
                    filesFiltered = files.filter((filepath) => exclusions?.test(filepath) === false);
                }
            }
            console.log("\x1b[36m%s\x1b[0m", "Converting images to alternative format...");
            if (filesFiltered.length > 0) {
                const actions = filesFiltered.map(async (inputFile) => this.processImage(inputFile, files));
                ...
                if (!converted && !this.conversion_error) {
                    ...
                    let code_search_files;
                    await fsp.appendFile(conversion_boolean_path, "");
                    if (this.options.codeGlob !== undefined &&
                        !this.options.overrideStagedFiles) {
                        code_search_files = (await globby(path.normalize(this.options.codeGlob))).filter((filepath) => stagedFilesList?.includes(filepath));
                    }
                    else if (this.options.codeGlob !== undefined) {
                        code_search_files = await globby(path.normalize(this.options.codeGlob));
                    }
                    if (code_search_files !== undefined && code_search_files.length > 0) {
                        await fsp.appendFile(code_search_files_path, code_search_files.join(os.EOL) + os.EOL);
                    }
                }
            }
            else {
    ...
    async processImage(inputFile, files) {
        const inputFileDir = path.dirname(inputFile);
        const inputFileBaseNameNoExt = path.basename(inputFile, path.extname(inputFile));
        ...
        try {
            ...
            console.error("\x1b[31m%s\x1b[0m", "Error processing metadata for image " + inputFile);
            console.error("\x1b[41m%s\x1b[0m", err);
        ...
        if (inputFileExt !== metadataFormat &&
            metadataFormat !== "raw" &&
        ...
        if (!files.some((filename) => filePaths.webpFilePath === filename ||
            Object.values(filePaths)
                .slice(8, -1)
                .some((newFileExt) => filename === newFileExt))) {
            image.webp({
                quality: 100,
                lossless: (metadataFormat === "gif" ||
                    metadataFormat === "bmp" ||
                    svgToLossless) &&
                    true,
                effort: 6,
            ...
            try {
                ...
                await fsp.appendFile(converted_source_image_files_path, inputFile + os.EOL);
                await fsp.appendFile(converted_target_image_files_path, filePaths.webpFilePath + os.EOL);
            }
            catch (err) {
                console.error("\x1b[31m%s\x1b[0m", "Error processing image " + inputFile);
                console.error("\x1b[41m%s\x1b[0m", err);
                this.conversion_error = true;
                try {
                    await fsp.appendFile(error_path, inputFile + os.EOL);
            ...
            if (!Object.values(filePaths)
                .slice(0, -9)
                .some((newFileExt) => files.some((filename) => newFileExt === filename))) {
                let imageFileName;
                ...
                try {
                    ...
                    console.log("\x1b[32m%s\x1b[0m", "Converted " + inputFile);
                    await fsp.appendFile(converted_source_image_files_path, inputFile + os.EOL);
                    await fsp.appendFile(converted_target_image_files_path, imageFileName + os.EOL);
                }
                catch (err) {
                    console.error("\x1b[31m%s\x1b[0m", "Error processing image " + inputFile);
                    console.error("\x1b[41m%s\x1b[0m", err);
